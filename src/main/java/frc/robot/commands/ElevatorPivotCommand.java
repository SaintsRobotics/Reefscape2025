...
public class ElevatorPivotCommand extends Command {
  private final ElevatorSubsystem m_elevatorSubsystem;
  private final EndEffectorSubsystem m_endEffectorSubsystem;

  private final double m_desiredHeight;
  private double m_desiredPivot;

  /** Creates a new ElevatorPivotCommand. */
  public ElevatorPivotCommand(double desiredHeight, double desiredPivot, ElevatorSubsystem elevatorSubsystem,
  ...
  public void execute() {
    ...
    final Entry<Double, List<Pair<Double, Double>>> currentLimit = EndEffectorConstants.kSafePivotPositions
        .floorEntry(currentPosition);
    final Entry<Double, List<Pair<Double, Double>>> higherLimit = EndEffectorConstants.kSafePivotPositions
        .higherEntry(currentLimit.getKey());
    final Entry<Double, List<Pair<Double, Double>>> lowerLimit = EndEffectorConstants.kSafePivotPositions
    ...
    double pivotPosition = m_desiredPivot;
    ...
    for (Pair<Double, Double> limit : currentLimit.getValue()) {
      ...
      final double min = Math.min(Math.abs(pivotPosition - limit.getFirst()),
          Math.abs(pivotPosition - limit.getSecond()));
      if (min < minDist) {
        minDist = min;
        closestLimit = limit;
      }
  ...
  public boolean isFinished() {
    return m_elevatorSubsystem.getHeightSetpoint() == m_desiredHeight && m_elevatorSubsystem.atSetpoint()
        && m_endEffectorSubsystem.getSetpoint() == m_desiredPivot && m_endEffectorSubsystem.atSetpoint();
  }
}
